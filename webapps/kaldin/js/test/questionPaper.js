function savetopic() {
	...
	if(document.getElementById("numberquestion").value != null && document.getElementById("numberquestion").value != '')
	...
	if(isError) {
		...
		function(data) {
			...
			if (data == null) {
				...
				//alert(totalQuestioninSubject);
				if(questioncount <= totalQuestioninSubject){
					//do nothing
				}else{
					if(subid != '' && tpcid == '' && level == ''){
						alert("Sorry entered question count is more than questions present in the category");
					}else if(subid != '' && tpcid != '' && level != ''){
						alert("Sorry entered question count is more than questions present in the complexity");
					}else if(subid != '' && tpcid != '' && level == ''){
						alert("Sorry entered question count is more than questions present in the sub category");
					}else if(subid != '' && tpcid == '' && level != ''){
						alert("Sorry entered question count is more than questions present in the complexity");
					}
					return false;
...
function calculateMark() {
	...
	var markperquest = '';
	var questionCount = '';
	if (document.getElementById("markperquestion").value != ''
			&& document.getElementById("numberquestiongiven").value != '') {
...
function deleteTopic(tpcid, type) {
	...
	function(data) {
		if (data == null) {
			...
			d = data.split("$$");
			document.getElementById("selectedtopic").innerHTML = "<center style=\"margin:40px 0 0 0;\"><b>Questions Added to Question Paper</b></center>" + d[1];
...
function getlevels(page) {
	...
	var topicid = 0;
	if (document.getElementById("topicidresponse") != null) {
		topicid = document.getElementById("topicidresponse").value;
	} else {
		topicid = 0;
	}
