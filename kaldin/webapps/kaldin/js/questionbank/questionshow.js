						...
						if (data == null) {
							...
							if (res.MCQ == 1) {
								...
								for (i = 1; i < 8; i++) {
									...
									if (ans[i] == "E" || ans[i] == "e") {
										document.getElementById("CorrectAnswerE").checked = true;
									}
									if (ans[i] == "F" || ans[i] == "f") {
										document.getElementById("CorrectAnswerF").checked = true;
									}
									if (ans[i] == "G" || ans[i] == "g") {
										document.getElementById("CorrectAnswerG").checked = true;
									}
...
function getChoice(choice) {
	...
	if(document.getElementById("singleAswerType").checked){
		...
		choice = 2;
	} else if(document.getElementById("subjectiveAswerType") != null && document.getElementById("subjectiveAswerType").checked){
		choice = 3;
	}
	if (totalOptions == 0 && choice != 3) {
		alert("Please Select Total Options First");
	...
	if (choice == 1) {
		$("#lblTotalOptions").show();
		getTextBoxesForChoice();
		...
		//hiding the question options
		$("#lblTotalOptions").hide();
	...
	if(document.getElementById("PlaceAnswerChoice"))
...
saveQuestion = function() {
	...
	var qType = document.getElementById("qtype").options[document.getElementById("qtype").selectedIndex].value;
	var qURL = '';
	
	if(qType == 1){
		qURL = document.getElementById("qtypeurl").value;
		if(qURL != null && qURL != ''){
			if(!isAudio(qURL)){
				alert("Enter valid audio url");
				return false;
			}
		}else{
			alert("Please enter valid audio url");
			return false;
		}
	}else if(qType == 2){
		qURL = document.getElementById("qtypeurl").value;
...
//			}
	...
	frm.questiontype.value = qType;
	frm.questionURL.value = qURL;
...
function showhideqtype(){
	var selValue = document.getElementById("qtype").options[document.getElementById("qtype").selectedIndex].value;
	if(selValue == 0){
		document.getElementById("questtype").style.display='none';
	}else{
		document.getElementById("questtype").style.display='';
	}
	if(selValue == 2){
		document.getElementById("typeSupport").innerHTML = '(You can provide the video links of Youtube or Vimeo or such alike.)';
	}else if(selValue == 1){
		document.getElementById("typeSupport").innerHTML = '(You can provide the audio links of mp3,wav or ogg format such alike.)';
	}
}

getTextBoxesForChoice = function() {
	var totalOptions = document.getElementById("TotalOptions").value;
	if (totalOptions == 2) {
		document.getElementById("optA").style.display='';
		document.getElementById("optB").style.display='';
